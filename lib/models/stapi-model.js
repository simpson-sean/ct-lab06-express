...
export default class stapi_model {
    ...
    static async insert({ name, species, faction, series}) {
        const { rows } = await pool.query(
            'INSERT INTO trek_characters (name, species, faction, series ) VALUES ($1, $2, $3, $4) RETURNING *',
            [ name, species, faction, series ]
        );
    ...
    static async updateCharacterById(id, { name, species, faction, series }) {
        ...
        const newSeries = series ?? existingCharacter.series;

        const { rows } = await pool.query(
            `UPDATE trek_characters SET name=$1, species=$2, faction=$3, series=$4 WHERE id=$5 RETURNING *`,
            [ newName, newSpecies, newFaction, newSeries, id ]
        );
